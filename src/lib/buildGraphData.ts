...
export interface GraphNode {
  ...
  slug: string;
  order: number;
  category: "core" | "foundation" | "area" | "protocol" | "principle" | "index";
  description: string;
  linkCount: number;
}
...
function deriveCategory(tags?: string[]): GraphNode["category"] {
  ...
  if (tags?.includes("area")) return "area";
...
export async function buildGraphData(): Promise<GraphData> {
  ...
  const nodes: GraphNode[] = pages
    .filter((p) => p.data.order !== 0 && p.data.order < 900)
    .map((p) => ({
      id: p.id,
      slug: toSlug(p.id),
      order: p.data.order,
  ...
  for (const page of pages) {
    if (page.data.order === 0 || page.data.order >= 900) continue;
