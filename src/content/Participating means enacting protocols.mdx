...
The network's shape emerges from below. Different members adopt different protocol clusters, creating a web where some connections are dense (many shared protocols) and others thinner (only the minimal set). No central authority determines the topology – it is the aggregate of freely adopted commitments. The [[Living systems become protocol domains|protocol domains]] show what these clusters look like in practice.
